...
class ReleaseWorkspaceStore {
  ...
  publishPlanToAgents(regionId, taskId, rolloutPercent) {
    ...
    const planRelease = {
      ...
      regionId, taskId, rolloutPercent: percent,
    ...
    const deliveries = enabledAgents.map((agent) => ({
      ...
      deliveredAt: createdAt, publishedBy: operator.id,
    }));
    ...
    this.#audit(
      ...
      planRelease.id,
      null,
      { taskId, rolloutPercent: percent, agentIds: planRelease.agentIds, checksum: bundle.checksum },
