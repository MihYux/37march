...
一个基于 Electron、React 和 TypeScript 构建的透明桌面伙伴原型。

她会以置顶、无边框窗口的形式待在桌面边缘，可以被拖到合适的位置；点击角色或快捷按钮会触发本地互动，展开聊天面板后可接入 DeepSeek 进行三月七风格的短对话，并使用阿里云百炼 CosyVoice 复刻音色朗读回复。

本仓库是更大桌宠项目中的角色形象与基础交互验证版本，当前重点是：

- 验证三月七的视觉形象、角色语气和桌面交互方式；
...
> [!IMPORTANT]
> 这是非官方同人原型，与米哈游 / HoYoverse 无隶属、合作或背书关系。角色及相关知识产权归原权利人所有。公开发布、商业使用或二次分发前，请自行核对最新二创规范及素材授权。
...
面向用户的“发行沙盒”和“Demo 控制中心”已经从主界面、托盘菜单和桌面路由中移除。底层数据兼容与历史测试暂时保留，避免旧数据迁移影响相册、通信和同行设置。
...
| 桌宠系统能力 | 可用 | 位置记忆、四边吸附、托盘/右键菜单、安全穿透、多屏保护和状态反馈 |
...
| PRD 原型验收 | 36/36 通过 | 机器可读矩阵、IPC 契约与三分钟完整演示均纳入 `npm run check` |
...
- 点击穿透仅在托盘恢复入口可用时允许开启，应用重启后强制恢复可点击；
- 支持从顶部拖动条、对话气泡、角色主体和透明空白区域拖动窗口；
- 角色区域采用 4 像素拖动阈值：短按触发互动，明显移动才开始拖窗；
- 按钮、输入框、聊天面板和设置面板不会误触发窗口拖动。

### 角色互动

- 点击角色：随机触发一条日常短句；
- 点击“拍照”：触发拍照主题的角色回复，不会直接写入长期记忆；
- 点击“收进相册”：明确确认后才把本次拍照保存为共同记忆；
- 点击“相册”：打开共同旅行相册；
- 点击“聊天”：打开或收起聊天面板；
- 根据内容切换四种情绪状态：
  - `bright`：元气、自然、亲切；
...
- 根据当前操作、未读通信、暂停和勿扰状态显示待机、拍照、打瞌睡、观察、新消息或记忆反馈；
- 关闭聊天或设置面板时可点击右上角关闭按钮，也可按 `Esc`。
...
- 每条记忆展示日期、摘要、三月七留言和用户确认状态；
- 可单独允许或禁止三月七未来引用某条记忆；
- 可关闭全部长期记忆；现有记录保留，但不会提供给角色或模型；
- 支持删除单条记忆和清空全部记忆；
- 删除记忆时同步移除通信消息中的可识别记忆 ID 引用；
- 支持将模拟记忆导出为 JSON，不会混入聊天、API Key 或发行任务；
...
| --- | --- |
| 齿轮 | 打开模型与语音设置 |
| 喇叭 | 开启 / 关闭语音；未配置语音时会引导进入设置 |
| 鼠标指针 | 开启点击穿透；必须从系统托盘恢复交互 |
| 图钉 | 切换窗口置顶状态 |
| 减号 | 最小化窗口 |
| 关闭 | 托盘可用时收起到系统托盘 |
...
- 在角色上按住并移动：拖动整个桌宠窗口；
- 单击“拍照”：触发拍照主题短句；
- 拍照后单击“收进相册”：确认保存为长期记忆；
- 单击“相册”：打开共同旅行相册；
- 单击“通信”：打开角色通信中心；数字角标表示未读数量；
- 单击“同行”：打开授权、频率、勿扰、暂停和退出设置；
...
> [!NOTE]
> 不要用 `npm run dev:renderer` 代替完整桌面启动。单独运行渲染页面只适合检查布局；浏览器环境没有 Electron IPC，因此模型设置、系统安全存储、窗口移动和语音播放会不可用。
...
| 窗口位置、尺寸、置顶和吸附 | Electron `userData` 目录中的 `window-state.json` | 持久保存；点击穿透不跨重启恢复 |
...
| 命令 | 用途 |
| --- | --- |
| `npm run dev` | 同时启动 Vite 和 Electron，完整开发模式 |
...
macOS 打包时如果没有 Developer ID，控制台会提示跳过代码签名；这是当前原型阶段的预期行为。正式发布前需要配置应用图标、签名、公证和更新机制。
...
│   ├── campaign-review.cjs      # 发行固定事实、受控生成与双层审核
│   ├── demo-scenarios.cjs       # 三类玩家、Demo 时钟与关系阶段
...
│   ├── PLATFORM_MATRIX.md
│   └── TROUBLESHOOTING.md        # 运行、模型、语音、Demo 与打包排障
├── scripts/
...
仓库中的 PNG 是为本原型制作的平面桌宠概念图，不是原 Live2D 模型、纹理或动作文件的复制品。
...
仍被明确标记为“原型可用、正式公开/商业分发前必须归档权利依据”，不能因为代码使用 MIT 就推断角色视觉也获得同样许可。
...
PRD 原型的 36 条验收、端到端演示、IPC 契约、可访问性基础和交付文档已经完成。下一步不再扩张原型范围，而是关闭正式发布门禁：
